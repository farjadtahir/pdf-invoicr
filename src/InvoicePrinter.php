...
class InvoicePrinter extends FPDF
{
    ...
    public function Header()
    {
        ...
        //First page
        if ($this->PageNo() == 1) {
            $dimensions = isset($this->dimensions[1]) ? $this->dimensions[1] : 0;
            ...
            if ($this->display_tofrom === true) {
                ...
                $this->SetTextColor(50, 50, 50);
                $this->SetFont($this->font, 'B', 10);
                $this->Cell($width, $lineheight, (isset($this->from[0]) ? $this->from[0] : 0), 0, 0, 'L');
                $this->Cell(0, $lineheight, iconv(self::ICONV_CHARSET_INPUT, self::ICONV_CHARSET_OUTPUT_A, (isset($this->to[0]) ? $this->to[0] : 0)), 0, 0, 'L');
